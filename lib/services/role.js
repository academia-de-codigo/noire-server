...
exports.addPermission = function(id, action, resourceName) {
    ...
    return Repository.tx(Repository.role.model, Repository.permission.model, Repository.resource.model,
        function(txRoleRepository, txPermissionRepository, txResourceRepository) {
            ...
            return txRoleRepository.findOne(id).eager('permissions').then(function(role) {
                ...
                return txResourceRepository.query().where('name', resourceName).then(function(resources) {
                    ...
                    return txPermissionRepository.query()
                        .where('action', action).andWhere('resource_id', resource.id).then(function(permissions) {
