...
//TODO: relate a role to multiple users, with an array
exports.addUser = function(id, userId) {
    ...
    return Repository.tx(Repository.role.model, Repository.user.model, function(txRoleRepository, txUserRepository) {
        ...
        var getUserById = txUserRepository.findOne(userId);

        return Promise.join(getRoleById, getUserById, function(role, user) {
            ...
            var duplicateUser = role.users.some(function(roleUser) {
...
//TODO: remove multiple users from a role, with an array
exports.removeUser = function(id, userId) {
    ...
    return Repository.tx(Repository.role.model, Repository.user.model, function(txRoleRepository, txUserRepository) {
        ...
        return Promise.join(getRoleById, getUserById, function(role, user) {
            ...
            var noUser = role.users.every(function(roleUser) {
                return roleUser.id !== user.id;
            });
...
exports.removePermission = function(id, permissionId) {

    return Repository.tx(Repository.role.model, Repository.permission.model, function(txRoleRepository, txPermissionRepository) {

        var getRoleById = txRoleRepository.findOne(id).eager('permissions');
        var getPermissionById = txPermissionRepository.findOne(permissionId);

        return Promise.join(getRoleById, getPermissionById, function(role, permission) {
            ...
            var noPermission = role.permissions.every(function(rolePermission) {
