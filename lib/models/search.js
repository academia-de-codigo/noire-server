...
internals.searchEntity = function(criteria) {
    // make sure we wrap an 'and' clause around all the search fields
    // https://vincit.github.io/objection.js/#sql-clause-precedence-and-parentheses
    this.where(builder => {
        this._modelClass.searchFields.forEach(field => {
            builder.orWhere(field, 'like', `%${criteria}%`);
        });
    });
...
internals.searchEntityWithRelations = function(criteria, relations) {
    ...
    return relationNames.reduce((queryBuilder, name) => {
        ...
        Hoek.assert(relation.ownerModelClass === queryBuilder._modelClass, 'entity is not the relation owner');
...
class SearchQueryBuilder extends QueryBuilder {
    ...
    search(criteria, relations) {
        ...
        return relations
            ? internals.searchEntityWithRelations.call(this, criteria, relations)
            : internals.searchEntity.call(this, criteria);
    }
}
