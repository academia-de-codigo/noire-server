...
internals.boomFromObj = function(error) {
    return () => new Boom(error.message, { statusCode: error.statusCode, data: { timestamp: Date.now() } });
};
...
internals.init = function() {
    ...
    for (let error in Errors) {
        ...
        exports[error] = isString ? internals.boomFromStr(errorObj) : internals.boomFromObj(errorObj);
