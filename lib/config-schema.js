...
internals.connectionSchema = Joi.object()
    .keys({
...
internals.redirectSchema = Joi.object()
    .keys({
        tlsOnly: Joi.array().items(
            Joi.object(internals.prefixesSchema(internals.prefixes)).required()
        )
    })
