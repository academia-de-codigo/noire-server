...
exports.register = function(server, options, next) {
    ...
    server.select('api').on('response', function(request) {
        ...
        // if response contains no data, no point in logging
        if (!response.source) {
            return;
        }

        // if response is html generated by the status-monitor plugin do not log
        if (request.url && request.url.path === '/status') {
            return;
        }

        // if response is a rendered view, do not log
