...
exports.ModelRepository = class {
    ...
    count(queryParams, subQuery) {
        ...
        if (queryParams && queryParams.search) {
            ...
            return countQuery.from(query.search(queryParams.search).as('ignored_alias')).first();
        }

        return subQuery ? countQuery.from(subQuery.as('ignored_alias')).first() : countQuery.first();
    }
...
exports.tx = async function(models, cb, trx) {
    ...
    models.forEach(model => Hoek.assert(model.prototype instanceof BaseModel, `invalid model ${model}`));
