...
internals.buildCriteria = function(Model, queryParams = {}) {
    ...
    criteria.relations = queryParams.relations;
...
const register = function(server, options) {
    ...
    options.models.forEach(model => {
        ...
        server
            .logger()
            .child({ plugin: exports.plugin.name, model: name })
            .debug('model');
    });
...
exports.ModelRepository = class {
    ...
    findAll(queryParams) {
        ...
        return this.model
            ...
            .search(criteria.search, criteria.relations) // uses custom SearchQueryBuilder
            .modify(query => {
                if (criteria.sort) {
     ...
     * @param {Object} [queryParams] the queryParams
     * @param {Objection.QueryBuilder} [query] the optional sub query to count from
     * @returns {Objection.QueryBuilder} the associated query
     */
    count(queryParams, subQuery) {
        const countQuery = this.model.query().count('* as count');

        if (queryParams && queryParams.search) {
            const query = subQuery || this.model.query();

            // aliasing with `as` is required for subquerying: http://knexjs.org/#Builder-as
            return countQuery
                .from(query.search(queryParams.search, queryParams.relations).as('ignored_alias'))
                .first();
        }

        return subQuery
            ? countQuery.from(subQuery.as('ignored_alias')).first()
            : countQuery.first();
    }
...
exports.create = function(model) {
    ...
    return new exports.ModelRepository(Model);
};
