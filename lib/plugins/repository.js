...
const BaseModel = require(Path.join(process.cwd(), 'lib/models/base'));

const internals = {
    modelsPath: Path.resolve(process.cwd(), 'lib/models'),
};

// limit default for rows returned when using findAll if Model does not provide one
internals.LIMIT_DEFAULT = 100;

internals.buildCriteria = function(Model, criteria) {

    // criteria comes from request object, make sure we do not mess with the original
    criteria = criteria ? Hoek.clone(criteria) : {};
    ...
    criteria.limit = criteria.limit || Model.LIMIT_DEFAULT || internals.LIMIT_DEFAULT;
    criteria.page = criteria.page || 1;
    criteria.descending = criteria.descending ? 'desc' : 'asc';
...
exports.ModelRepository = class {
     ...
     * @param {(number|string|Object)} [criteria] the query criteria
     * @returns {Objection.QueryBuilder} the associated query
     */
    findAll(criteria) {

        criteria = internals.buildCriteria(this.model, criteria);
        return this.model.query()
            ...
            .modify(query => {
                if (criteria.sort) {
                    query.orderBy(criteria.sort, criteria.descending);
                }
            });
    }
    ...
    count(criteria) {
        ...
        if (typeof criteria === 'object' && criteria.search) {
            ...
            countQuery.from(this.model.query().search(criteria.search).as('ignored_alias'));
        }
