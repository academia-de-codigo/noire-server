...
const internals = {
    ...
    audience: {
        ...
        passwordReset: 'noire:password-reset'
    }
};

//TODO: refactor get/decode token methods
...
/**
 * Gets a signed jwt password reset token
 * @async
 * @param {string} id the user id
 * @returns {Promise<string>} the token
 */
exports.getPasswordResetToken = function(id) {
    // token options
    const options = {
        audience: [internals.audience.passwordReset],
        expiresIn: Config.auth.passwordResetIn
    };
...
/**
 * Verifies and decodes a jwt password reset token
 * @param {string} token the jwt password reset token
 */
exports.decodePasswordResetToken = async function(token) {
    const decode = Util.promisify(JWT.verify);
    const secret = new Buffer(process.env.JWT_SECRET, 'base64');
