...
const register = async function(server) {
    ...
    server.ext('onPreStop', server => {
        ...
        server.logger().info({ plugin: exports.plugin.name }, 'stop');
    });
    ...
    server.logger().info({ plugin: exports.plugin.name }, 'start');
    server.logger().debug({ plugin: exports.plugin.name, config: internals.config }, 'start');
};
