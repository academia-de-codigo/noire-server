 ...
 * @returns {Promise<Resource>} the updated resrouce
 */
exports.update = function(id, entity) {

    // transaction makes sure we do not get resources with the same name
    return Repository.tx(Repository.resource.model, async txResourceRepository => {

        const getResourceById = txResourceRepository.findOne(id);
        const getDuplicateResources = txResourceRepository.query().where('name', entity.name);

        const results = await Promise.all([getResourceById, getDuplicateResources]);
        const resource = results[0];
        const duplicateResources = results[1];
