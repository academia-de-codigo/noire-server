...
exports.count = async function(criteria) {
    const result = await Repository.user.count(criteria);
    return result.count;
};
...
exports.findByUserName = async function(username) {
    ...
    const user = await Repository.user.query().findOne({ username }).omit(['password']).eager('roles');
...
exports.findByEmail = async function(email) {
    ...
    const user = await Repository.user.query().findOne({ email }).omit(['password']);
...
exports.add = function(entity) {
    ...
    return Repository.tx(Repository.user.model, async txUserRepository => {
        ...
        const getEqualUsers = txUserRepository.query().where('username', entity.username).orWhere('email', entity.email);

        const results = await Promise.all([getEqualUsers, getHash]);
        const equalUsers = results[0];
...
exports.update = function(id, entity) {
    ...
    // transaction makes sure we do not end up with
    // users with the same email or username
    return Repository.tx(Repository.user.model, async txUserRepository => {

        const getUserByID = txUserRepository.findOne(id);
        const getEqualUsers = txUserRepository.query().skipUndefined()
            .where('username', entity.username).orWhere('email', entity.email);

        const results = await Promise.all([getUserByID, getEqualUsers]);
        const user = results[0];
        const equalUsers = results[1];
