...
exports.list = async function(request, h) {
    ...
    const [totalCount, results] = await Promise.all([UserService.count(request.query), UserService.list(request.query)]);
 ...
 * @param {Request} request the request object
 * @param {Toolkit} h the response toolkit
 * @returns {Response} the response objec containing the created user
 */
