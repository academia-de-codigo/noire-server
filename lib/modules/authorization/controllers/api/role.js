...
exports.list = async function(request, h) {
    ...
    const [totalCount, results] = await Promise.all([RoleService.count(request.query), RoleService.list(request.query)]);
 ...
 */
exports.addUsers = async function(request) {

    // path params are passed as strings
    const [id, userIds] = [Number.parseInt(request.params.id), request.payload.id];

    return await RoleService.addUsers(id, userIds);
};
 ...
 * @async
 * @param {Request} request the request object
 * @returns {Response} the response object containing ???
 */
exports.removeUsers = async function(request) {

    // path params are passed as strings
    const [id, userIds] = [Number.parseInt(request.params.id), request.payload.id];

    return await RoleService.removeUsers(id, userIds);
};
 ...
 */
exports.addPermissions = async function(request) {
    ...
    return await RoleService.addPermissions(id, action, resource);
};
 ...
 * @async
 * @param {Request} request the request object
 * @returns {Response} the response object containing the number of mappings removed
 */
exports.removePermissions = async function(request) {

    // path params are passed as strings
    const [id, permIds] = [Number.parseInt(request.params.id), request.payload.id];

    return await RoleService.removePermissions(id, permIds);
};
