...
internals.createTransport = async function() {
    ...
    try {
        ...
        if (Config.smtp.test) {
            const transportVerifier = Util.promisify(transporter.verify);
            await transportVerifier();
        }
...
internals.compileTemplates = async function() {
    const templatePath = Path.join(process.cwd(), Config.mail.templates);
    const templates = {};
...
exports.sendMail = async function(email) {
    ...
    try {
        const info = await internals.sendMail({
            from,
            to,
            subject,
            html,
...
exports.init = async function() {
    ...
    internals.sendMail = Util.promisify(internals.transporter.sendMail).bind(internals.transporter);
};
